...
app.get("/api/bots/:id", async (req, res) => {
  const data = await global.botModel.findOne({ id: req.params.id });
  if (!data) return res.status(404).json({ message: `This bot is not in our system.` });
  if (!data.approved) return res.status(409).json({ message: `The bot requested is not approved.` });
  return res.json({
    prefix: data.prefix,
    owner: data.owner,
