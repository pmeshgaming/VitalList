...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!server)
      return interaction.repy("This server was not found on VitalServers.");
    ...
    if (cooldown - (Date.now() - lastDaily) > 0) {
      ...
      const ServerRaw =
        (await global.sclient.guilds.fetch(interaction.guild.id)) || null;
      ...
      const votedEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "<:vote:1028862219313762304> " +
            server.name +
            " has been bumped on VitalServers."
        )
        .setColor("Aqua")
        .addFields({
          name: "Server",
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          "You have successfully bumped this server to the top of [VitalServers](https://vitallist.xyz/servers)."
        )
        .setFooter({
          text: `VitalServers - Bump Command`,
          iconURL: `${global.sclient.user.displayAvatarURL()}`,
        });
